...
public class waveThree
{
    ...
    private Rectangle parasiteRect;
    private Image parasite, explosion;
    ...
    public waveThree(int xDimension)
    {
        try {
        ...
        width = 50;
        height = width;
        x = xDimension*width+275;
        y = height+100;
        originalXPos = xDimension*width+275;
        originalYPos = height+100;
        Vel = -1;
        yVel = 0;

        parasite = Toolkit.getDefaultToolkit().getImage(getClass().getResource("waveThree.gif"));
        parasite = parasite.getScaledInstance(50, 50, 1);
        ...
        parasiteRect = new Rectangle (x, y, width, height);
    ...
    public void draw(Graphics2D g2)
    {
        for (int index = 0; index < ammo.length; index++)
            ammo[index].draw(g2);
        if (dead == false)
            g2.drawImage(parasite, x, y, null);
        else
    ...
    public void die()
    {
        ...
        //Vel = 0;
        yVel = 0;
        parasiteRect.setLocation(1000, 1000);
    }
